<template>
  <section class="w-full max-w-6xl mx-auto px-3 sm:px-4 md:px-6 py-4 sm:py-6 space-y-5 sm:space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-3" v-if="hasData">
      <article
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-4" v-if="hasData">
      <article class="rounded-xl border bg-white p-4 shadow-sm">
        ...
        <p v-else class="text-sm text-gray-500">Нет заявок по статусам за выбранный период.</p>
      </article>

      <article class="rounded-xl border bg-white p-4 shadow-sm">
        <div class="flex items-center justify-between gap-3 mb-4">
          <h3 class="text-base font-semibold text-gray-900">Источники заявок</h3>
          <span class="text-xs text-gray-500">по созданным заявкам</span>
        </div>
        ...
        <p v-else class="text-sm text-gray-500">Нет данных по источникам за выбранный период.</p>
      </article>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-4" v-if="hasData">
      <article class="rounded-xl border bg-white p-4 shadow-sm">
        <div class="flex items-center justify-between gap-3 mb-4">
          <h3 class="text-base font-semibold text-gray-900">Выданные устройства и выручка</h3>
          ...
          <table class="min-w-full text-xs sm:text-sm border-collapse">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="p-2 sm:p-3 border text-left whitespace-nowrap">Выдано</th>
            ...
            <tbody>
              <tr v-for="item in items" :key="item.product">
                ...
                <td class="p-2 sm:p-3 border whitespace-nowrap">{{ formatMoney(item.price) }}</td>
                <td class="p-2 sm:p-3 border">{{ item.count }}</td>
                <td class="p-2 sm:p-3 border font-semibold whitespace-nowrap">{{ formatMoney(item.sum) }}</td>
              </tr>
              <tr>
                <td class="p-2 sm:p-3 border font-bold" colspan="3">Итого</td>
                <td class="p-2 sm:p-3 border font-bold whitespace-nowrap">{{ formatMoney(total) }}</td>
              </tr>
            </tbody>
          </table>
        </div>

        <p v-else class="text-sm text-gray-500">Нет выданных устройств за выбранный период.</p>
      </article>

      <article class="rounded-xl border bg-white p-4 shadow-sm">
        <div class="flex items-center justify-between gap-3 mb-4">
          <h3 class="text-base font-semibold text-gray-900">Топ школ</h3>
          <span class="text-xs text-gray-500">по новым заявкам</span>
        </div>

        <div v-if="schoolItems.length" class="space-y-3">
          <div v-for="item in schoolItems" :key="item.school" class="space-y-1">
            <div class="flex items-center justify-between gap-3 text-sm">
              <span class="text-gray-700 break-words">{{ item.school }}</span>
              <span class="font-semibold text-gray-900">{{ item.count }}</span>
            </div>
            <div class="h-2 rounded-full bg-gray-100 overflow-hidden">
              <div
                class="h-full rounded-full bg-emerald-500"
                :style="{ width: `${schoolShare(item.count)}%` }"
              />
            </div>
          </div>
        </div>

        <p v-else class="text-sm text-gray-500">Нет школ с заявками за выбранный период.</p>
      </article>
    </div>
    ...
    <article class="rounded-xl border bg-white p-4 shadow-sm" v-if="hasData">
      ...
      <div class="overflow-x-auto" v-if="dailyItems.length">
        <table class="min-w-full text-xs sm:text-sm border-collapse">
          <thead class="bg-gray-50">
            <tr>
              <th class="p-2 sm:p-3 border text-left whitespace-nowrap">Дата</th>
              <th class="p-2 sm:p-3 border text-left whitespace-nowrap">Создано</th>
              <th class="p-2 sm:p-3 border text-left whitespace-nowrap">Выдано</th>
          ...
          <tbody>
            <tr v-for="item in dailyItems" :key="item.date">
              <td class="p-2 sm:p-3 border whitespace-nowrap">{{ formatDate(item.date) }}</td>
...
const maxStatusCount = computed(() => Math.max(...statusItems.value.map((item) => item.count), 0))
const maxSchoolCount = computed(() => Math.max(...schoolItems.value.map((item) => item.count), 0))
const maxSourceCount = computed(() => Math.max(...sourceItems.value.map((item) => item.count), 0))
...
async function load() {
  ...
  try {
    const response = await $fetch<DashboardResponse>(`/api/sales-report?from=${from.value}&to=${to.value}`)
